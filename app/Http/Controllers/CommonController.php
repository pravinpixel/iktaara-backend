...
class CommonController extends Controller
{
    ...
    public function filterAddressDropdown(Request $request){
        
        $stateId = $request->input('state_id');
        $areaId = $request->input('area_id');   

        if(isset($stateId) && isset($areaId)){
            return filterJsonPincode($stateId, $areaId);
        }else{
            return filterJsonArea($stateId);
        }
    }
}
