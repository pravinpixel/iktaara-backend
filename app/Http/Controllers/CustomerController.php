...
class CustomerController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = Customer::select('customers.*')->orderBy('created_at', 'desc');
            $filter_subCategory   = '';
    ...
    public function modalAddEdit(Request $request)
    {
        ...
        if (isset($id) && !empty($id)) {
            ...
            $default_customer_address = CustomerAddress::where([['customer_id', $info->id], ['is_default', 1]])->first();
            $modal_title    = 'Update Customer';
        }

        return view('platform.customer.add_edit_modal', compact('info','state', 'modal_title', 'default_customer_address'));
    }
    ...
    public function saveForm(Request $request, $id = null)
    {
        ...
        $validator      = Validator::make($request->all(), [
            ...
            'mobile_no' => 'required|numeric|digits:10|unique:customers,mobile_no,'.$id.',id,deleted_at,NULL',
        ]);
        if ($validator->passes()) {
            ...
            $error                      = 0;
                ...
                if( $customerId){
                    $default_customer_address =  new CustomerAddress;
                    $default_customer_address->customer_id =$customerId;
                    $default_customer_address->address_line1 = $request->address_line1;
                    $default_customer_address->address_line2 = $request->address_line2;
                    $default_customer_address->city = $request->city;
                    $default_customer_address->state = $request->state;
                    $default_customer_address->post_code = $request->post_code;
                    $default_customer_address->is_default =1;
                    $default_customer_address->save();
                }
            if($id){
                $default_customer_address = CustomerAddress::where([['customer_id', $id], ['is_default', 1]])->first();
                    $default_customer_address->address_line1 = $request->address_line1;
                    $default_customer_address->address_line2 = $request->address_line2;
                    $default_customer_address->city = $request->city;
                    $default_customer_address->state = $request->state;
    ...
    public function customerChangePassword(Request $request, $id = null)
    {
        $id = $request->id ?? '';
