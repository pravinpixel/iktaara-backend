        ...
        @endphp
            {{-- @foreach ($order_info->orderItems as $item) --}}
                <tr>
                    ...
                    {{-- <td>15200</td> --}}
                    <td> {{ $orderItems->quantity }} nos</td>
                    <td> {{ number_format($order_info->seller_price - (($order_info->profit_margin_percentage /100) * $order_info->seller_price), 2) }} </td>
                    {{-- <td>0%</td> --}}
                    {{-- <td>{{ $orderItems->tax_percentage / 2 }}%</td>
                    <td>{{ number_format(($orderItems->tax_amount / 2), 2) }}</td>
                    <td>{{ $orderItems->tax_percentage / 2 }}%</td>
                    <td>{{ number_format(($orderItems->tax_amount / 2), 2) }}</td> --}}
                    <td>{{ number_format(($order_info->total - ($order_info->profit_margin_percentage /100) * $order_info->total), 2) }}</td>
                </tr>
            {{-- @endforeach --}}
        @endif
    </table>
    <table class="item-table" cellspacing="0" padding="0">
        <tr>
            <td style="padding-top:10px;width:50%;border-bottom:none;">
                ...
                <div>
                    <b>{{ ucwords( getIndianCurrency(($order_info->total - ($order_info->profit_margin_percentage /100) * $order_info->total)) ) }}</b>
                </div>
            ...
            <td style="width: 50%;">
                <table class="no-border" cellspacing="0" padding="0" style="width: 100%;">
                    {{-- <tr>
                        ...
                        {{-- </td>
                        <td style="text-align: right"><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{ number_format(($order_info->total - ($order_info->profit_margin_percentage /100) * $order_info->total), 2) }}</td>
                    </tr> --}}
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;font-weight:700;font-size:15px;">
                            <span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span>{{ number_format(($order_info->total - ($order_info->profit_margin_percentage /100) * $order_info->total), 2) }}</td>
                    </tr>
                    <tr>
