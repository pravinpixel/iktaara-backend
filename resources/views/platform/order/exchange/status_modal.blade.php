                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="fv-row mb-7">
                                    <label class="required fw-bold fs-6 mb-2">Exchange Status</label>
                                    <select class="form-select form-select-solid" name="status" id="status">
                                        @if(isset($orderExchange) && !empty($orderExchange))
                                            @foreach ($orderStatuses as $index => $orderStatus)
                                                <option value="{{ $index }}" @if($orderExchange[0]->status === $index) selected @endif>{{ $orderStatus }}</option>
                                            @endforeach
                                        {{-- @else
                                            @foreach ($orderStatuses as $index => $orderStatus)
                                                <option value="{{ $index }}">{{ $orderStatus }}</option>
                                            @endforeach --}}
                                        @endif
                                    </select>
                                </div>
...
<script>
    var add_url = "{{ route('order.exchange.status-update') }}";
    ...
    $(document).ready(function(){
        ...
        // Initial call to update options based on the initial state
        updateOptions();

        // Attach a change event handler to update options whenever the selection changes
